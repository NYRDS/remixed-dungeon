    ...
    def _check_links(self, content: str):
        ...
        for link in potential_links:
            ...
            if namespace_pos != -1:
                ...
                standard_namespaces = ['rpd', 'ru', 'cn', 'es', 'pt', 'code', 'playground', 'doku', 'wiki', 'wp', 'en', 'zh', 'fr', 'de', 'it', 'ja', 'ko', 'pl', 'uk', 'hu', 'tr', 'in', 'ms', 'el']
                is_standard = (namespace in standard_namespaces or
                              namespace.startswith('mr') or
                              (namespace == 'en' and page_name.startswith('rpd:')))
            ...
            if not tag_pattern.match(tag):
                ...
                for t in individual_tags:
                    if t:
                        ...
                        elif t.startswith(':') and (t[1:] == 'rpd' or t[1:].startswith('ru:') or t[1:].startswith('cn:') or t[1:].startswith('es:') or t[1:].startswith('pt:') or t[1:].startswith('en:') or t[1:].startswith('zh:') or t[1:].startswith('fr:') or t[1:].startswith('de:') or t[1:].startswith('it:') or t[1:].startswith('ja:') or t[1:].startswith('ko:') or t[1:].startswith('pl:') or t[1:].startswith('uk:') or t[1:].startswith('hu:') or t[1:].startswith('tr:') or t[1:].startswith('in:') or t[1:].startswith('ms:') or t[1:].startswith('el:')):
                            continue
                        else:
                            valid = False
                            break
