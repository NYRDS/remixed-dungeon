...
def is_jsonl_file(filepath: str) -> bool:
    """Check if a file is in JSONL format (one JSON value per line)."""
    path = Path(filepath)
    # Localization string files use JSONL format
    if path.name.startswith("strings_") and path.name.endswith(".json"):
        return True
    return False
...
def validate_json_file(filepath: str) -> Tuple[bool, str]:
    ...
    try:
        with open(filepath, "r", encoding="utf-8") as f:
            ...
            # Handle JSONL files (one JSON value per line)
    ...
    if "map" in data:
        if not isinstance(data["map"], list):
            ...
            if len(data["map"]) != expected_height:
                errors.append(
                    f"Map height ({len(data['map'])}) doesn't match height field ({expected_height})"
                )
            ...
            for i, row in enumerate(data["map"]):
                if isinstance(row, list):
                    if len(row) != expected_width:
                        errors.append(
                            f"Row {i} width ({len(row)}) doesn't match width field ({expected_width})"
                        )
...
def main():
    project_path = (
        sys.argv[1]
        if len(sys.argv) > 1
        else "/home/mike/StudioProjects/remixed-dungeon"
    )
        ...
        if is_valid:
            ...
            try:
                with open(json_file, "r", encoding="utf-8") as f:
                    ...
                    # Skip specific validation for JSONL files (localization strings)
                    if is_jsonl_file(json_file):
                        valid_count += 1
                        print(f"✓ {json_file} (JSONL format)")
                        continue
                ...
                if not specific_errors:
                    ...
                    errors.append(
                        (
                            json_file,
                            f"Specific validation failed: {'; '.join(specific_errors)}",
                        )
                    )
                    print(
                        f"✗ {json_file} - Specific validation failed: {'; '.join(specific_errors)}"
                    )
            except Exception as e:
