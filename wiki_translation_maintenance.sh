...
while true; do
    ...
    if [ $TASK_CHOICE -eq 0 ]; then
        ...
        COMMAND="qwen -y 'read @docs/WIKI_DOCUMENTATION.md, pull repo master, pick 5 random wiki pages using tools/pick_random_wiki_pages.sh, analyze them for compliance with wiki standards, identify issues like missing images, invalid headers, incorrect links, improper formatting, run dokuwiki linter on them, fix identified issues based on documentation standards, verify all links point to existing lowercase files, ensure proper image references exist, commit your changes, push it. Focus on maintaining consistency with wiki documentation standards.'"
