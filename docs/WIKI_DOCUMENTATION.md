...
- The processed sprites maintain the exact same filename as input sprites
- All output sprites are in PNG format with scaling (8x by default), background, and frame applied

#### Wiki Integration Naming
For wiki page integration, the sprites are referenced with specific suffixes:
- For most entities: `{{ rpd:images:[filename]_sprite.png|Alt Text }}`
- For items specifically: `{{ rpd:images:[itemname]_item.png|Alt Text }}`
...
#### Directory Structure
- Raw sprites: `/sprites/` (generated by FactorySpriteGenerator)
- Processed sprites for wiki: `/wiki-data/media/rpd/images/`
