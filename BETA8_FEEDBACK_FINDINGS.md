...
### A9 update — BoneSaw SHIPPED (rulings by Mike: stays non-upgradable — it scales with the doctor; rich info; real STR req)
`scripts/items/BoneSaw.lua` already had level-scaled damage/accuracy/delay (`damageRoll = (user_level+lvl)*random(2+lvl, 3+2*lvl)`, bleed, crit ×1.5, Doctor harvest on crit). Shipped changes (lua-only): authored `requiredSTR = 9` made mechanically real — STR deficit now penalizes via the standard Weapon formulas (`/1.5^n` accuracy, `×1.2^n` delay) in the item's own accuracy/delay hooks; rich `info` hook shows the effective average damage (reuses localized `MeleeWeapon_Info2a`) and the "too heavy" line (`MeleeWeapon_Info2c`); the bogus `statsRequirementsSatisfied`/`knownStatsText`/`unknownStatsText` overrides removed in favor of the java base. Still open quirk: `attackProc` applies **×2** vs paralyzed foes but the perk text promises **×1.5** — unruled, code left as is.
