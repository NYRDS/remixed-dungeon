...
for root, dirs, files in os.walk(values_dir):
    ...
    os.makedirs("l10ns", exist_ok=True)
    jsonData = open(f"l10ns/strings_{locale_code}.json", "w", encoding='utf8')
    for file in files:
...
for locale in locales:
    # Create the directory if it doesn't exist
    os.makedirs("l10ns", exist_ok=True)
    jsonData = open(f"l10ns/strings_{locale}.json", "w", encoding='utf8')
