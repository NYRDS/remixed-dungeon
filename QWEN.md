...
- In Remixed Dungeon, the getEntityKind method serves as the central identification mechanism for game entities. It is part of the NamedEntityKind interface and is implemented by actors, items, buffs, and other entities. The method connects Java game engine with JSON configuration files (like "mobsDesc/" + getEntityKind() + ".json") and Lua scripting system. For standard entities, it typically returns the class simple name, while for custom entities it can return script file names. Lua scripts use getEntityKind() to identify entity types during gameplay, enabling modding capabilities where new entities can be added via JSON configs and Lua scripts while maintaining compatibility with existing game systems.
