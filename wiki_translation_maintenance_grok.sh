...
while true; do
    ...
    if [ $TASK_CHOICE -eq 0 ]; then
        ...
        COMMAND="grok -p 'read @docs/WIKI_DOCUMENTATION.md, pull repo master, pick 5 random wiki pages using tools/pick_random_wiki_pages.sh, analyze them for compliance with wiki standards, identify issues like missing images, invalid headers, incorrect links, improper formatting, run dokuwiki linter on them, fix identified issues based on documentation standards, verify all links point to existing lowercase files, ensure proper image references exist, for mr: namespace pages use tools/find_entity_usage.py to analyze entity implementation and usage in code, also check wiki pages in all supported languages (en, ru, es, fr, de, it, pl, pt-rBR, ja, ko, zh-rCN, zh-rTW, uk, hu, tr, el, in, ms) to ensure consistency with game translations, commit your changes to wiki-data and push it, do not commit or push into main repo. Focus on maintaining consistency with wiki documentation standards.' --yolo"
        TASK_NAME="Wiki maintenance"
    else
        # Translation task
        COMMAND="grok -p 'read @docs/TRANSLATION_TASK.md, pull repo master, identify few random missing strings in random languages (use tools/select_random_missing_string.py), find their context using tools/find_string_usage.py, translate them properly based on English reference and code context, add translations to appropriate strings_all.xml files using tools/insert_translated_string.py, verify consistency with existing translations, run tools/validate_translations.py to check for formatting issues, also select few random strings and ensure consistency among all languages, commit your changes, push it. Focus on maintaining consistency with existing translations, proper grammar, cultural appropriateness for target languages, and proper string formatting following Android XML standards.' --yolo"
