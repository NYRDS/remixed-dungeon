...
# Uses Grok (headless) instead of Qwen as the AI agent
#
# Important: do NOT store the full CLI line in a string and eval it.
# Unquoted expansion word-splits the prompt; embedded quotes become literal characters,
# so grok sees "pull" as a separate argument (error: unexpected argument 'pull' found).
# Pass the prompt as a properly quoted shell variable instead.
# Grok auto-approve flag is --always-approve (not qwen's -y/--yolo).

set -u

echo "Maintenance script that randomly chooses between wiki and translation tasks (Grok agent)"
echo "Will run a random task, sleep for 1 hour, and repeat indefinitely."
echo "Press Ctrl+C to stop."
echo

WIKI_PROMPT='read @docs/WIKI_DOCUMENTATION.md, pull repo master, pick 5 random wiki pages using tools/pick_random_wiki_pages.sh, analyze them for compliance with wiki standards, identify issues like missing images, invalid headers, incorrect links, improper formatting, run dokuwiki linter on them, fix identified issues based on documentation standards, verify all links point to existing lowercase files, ensure proper image references exist, for mr: namespace pages use tools/find_entity_usage.py to analyze entity implementation and usage in code, also check wiki pages in all supported languages (en, ru, es, fr, de, it, pl, pt-rBR, ja, ko, zh-rCN, zh-rTW, uk, hu, tr, el, in, ms) to ensure consistency with game translations, commit your changes to wiki-data and push it, do not commit or push into main repo. Focus on maintaining consistency with wiki documentation standards.'

TRANSLATION_PROMPT='read @docs/TRANSLATION_TASK.md, pull repo master, identify few random missing strings in random languages (use tools/select_random_missing_string.py), find their context using tools/find_string_usage.py, translate them properly based on English reference and code context, add translations to appropriate strings_all.xml files using tools/insert_translated_string.py, verify consistency with existing translations, run tools/validate_translations.py to check for formatting issues, also select few random strings and ensure consistency among all languages, commit your changes, push it. Focus on maintaining consistency with existing translations, proper grammar, cultural appropriateness for target languages, and proper string formatting following Android XML standards.'
...
while true; do
    ...
    echo "[$(date)] Running $TASK_NAME"
    echo "[$(date)] Prompt: $PROMPT"

    # -p: headless single prompt; --always-approve: auto-approve tools (qwen -y equivalent)
    # Keep prompt in "$PROMPT" so the full text is one argv to grok.
    timeout 30m time grok -p "$PROMPT" --always-approve
    CMD_STATUS=$?
