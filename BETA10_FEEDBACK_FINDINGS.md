...
**Findings:** two parts.
*The drink is legal and not free:* `MobItemAi` scores PotionOfHealing in COMBAT by hp deficit (`MobItemAi.java:98-103`) — the potion was in its belongings — and `Potion.drink` spends a full turn (`doOperate(TIME_TO_DRINK)` → `spend(1.0)`, `Potion.java:195-205`). No same-turn double action in code; the attack came on the kobold's next activation.
*The impossible hit is real:* `KoboldIcemancer.canAttack` is `Ballistica.cast(...) == enemyPos` with **no distance cap** (`KoboldIcemancer.java:41-43`), and `Level.distance` is Chebyshev (`Level.java:1612-1618`). At diagonal distance 1 the single Bresenham step lands directly on the hero — the two flanking corner walls are never sampled → `canAttack` true **through a diagonal wall corner**. `doAttack` with distance ≤ 1 plays the *melee* animation (`Char.java:1234-1239`) — hence "hit me, didn't shoot". Same quirk exists for every mob (`Char.canAttack`: adjacent → true, `Char.java:1977-1985`) but is glaring for a caster that otherwise zaps.
