...
COMMAND="qwen -y  'read @docs/WIKI_DOCUMENTATION.md, pull repo and wiki-data master branches prefer remote changes, resolve conflicts in wiki-data if necessary, pick few random wiki pages and rigorously verify it against codebase (java, lua and json configs) insert references to data sources(code, configs, string resources), wikify (create wiki links, make sure to use proper naming scheme and dokuwiki syntax), check for redlinks and create new pages if necessary (make sure to create new pages with proper naming scheme only!), check for any obsolete, incorrect or duplicated info and fix it, commit changed files in wiki-data one by one, push changes to wiki-data master branch. Don't create additional scripts or tools on this step, however you cloud document your suggestions on additional tools on ai_wiki wiki page.'"
