...
public class PseudoGlyphLayout implements Poolable {
	...
	public PseudoGlyphLayout() {
	}


	public void setText (BitmapFontData font, CharSequence str) {
		setText(font, str, 0, str.length(), Color.BLACK, 0, Align.left, false, null);
	}

	public void setText (BitmapFontData fontData, CharSequence str, Color color, float targetWidth, int halign, boolean wrap) {
		setText(fontData, str, 0, str.length(), color, targetWidth, halign, wrap, null);
	}
	public void setText (BitmapFontData fontData, CharSequence str, int start, int end, Color color, float targetWidth, int halign,
		...
		while (true) {
			...
			runEnded:
			{
				...
				if (run.glyphs.size == 0) {
					...
					lineRun.glyphs.addAll(run.glyphs);
					// Remove the width of the last glyph. The first xadvance of the appended run has kerning for the last glyph of this run.
					if (lineRun.xAdvances.notEmpty()) lineRun.xAdvances.size--;
					lineRun.xAdvances.addAll(run.xAdvances);
